...
def crearSConstruct(dirname):

        SConstruct = open(os.getcwd()+'/'+dirname+'/SConstruct','w')
	
	SConstruct.write("""#!/usr/bin/python
...
envArduino.Append(BUILDERS = {'Processing':Builder(action = fnProcessing, suffix = '.cpp', src_suffix = FILE_EXTENSION)})
envArduino.Append(BUILDERS = {'CompressCore':Builder(action = fnCompressCore) })
envArduino.Append(BUILDERS={'Elf':Builder(action=AVR_BIN_PREFIX+'gcc -mmcu=%s -Os -Wl,--gc-sections -o $TARGET $SOURCES -lm'%MCU)})
envArduino.Append(BUILDERS={'Hex':Builder(action=AVR_BIN_PREFIX+'objcopy -O ihex -R .eeprom $SOURCES $TARGET')})
